...
class ChartAnnotationBuilder {
  ...
  List<ChartAnnotation<ChartObject>>? buildAnnotations({
    ...
    String? currentAsset,
    List<OpenedPosition> openedPositions = const [],
  }) {
    if (candles.isEmpty) {
      return null;
    }
    ...
    // 添加持仓入场价 Y 轴锚点（不绘制，仅参与 Y 轴范围计算）
    _addOpenedPositionYAxisAnchors(
      chartAnnotations: chartAnnotations,
  ...
  /// 添加持仓入场价的 Y 轴锚点
  /// 当开启 Y 轴自适应（[entryPriceYAxisAdaptive]）时，为每个当前资产的
  /// 持仓添加一条隐藏的 [HorizontalBarrier]，让图表自动扩大 Y 轴范围，
  /// 确保入场价始终落在可见区域内。
  void _addOpenedPositionYAxisAnchors({
    ...
    if (!entryPriceYAxisAdaptive || openedPositions.isEmpty) {
      return;
    }

    final asset = currentAsset?.toUpperCase();
    for (final position in openedPositions) {
      if (asset != null &&
          position.tradingPair.baseAsset.toUpperCase() != asset) {
        continue;
      }
      chartAnnotations.add(HorizontalBarrier(
        position.entryPrice,
        id: 'opened_position_y_anchor_${position.id}',
