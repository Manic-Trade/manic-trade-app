...
class CandleDataSynchronizer {
  ...
  /// 同步操作的代数，用于处理并发调用：
  /// 每次 _loadDataWithSocketSync 被调用时递增，
  /// await 后检查是否被新调用取代，避免并发竞争
  ...
  Future<void> stop() async {
    ...
    _isStarted = false;
    // 使正在执行的 _loadDataWithSocketSync 失效
    _syncGeneration++;
  ...
  Future<void> dispose() async {
    ...
    // 先执行 stop 清理（stop 需要 _isDisposed=false 才能进入）
  ...
  Future<void> _loadDataWithSocketSync(
    ...
    // 递增 generation，使之前可能还在执行的并发调用失效
    ...
    if (sequential) {
      ...
      await _subscribeSocket(generation: generation);
      // 被新调用取代，退出
      if (_syncGeneration != generation) return;
      await _loadInitialData(generation: generation, isRefresh: isRefresh);
    } else {
      // 并行执行：socket 连接和接口请求同时进行
      await Future.wait([
        _subscribeSocket(generation: generation),
        _loadInitialData(generation: generation, isRefresh: isRefresh),
      ]);
      // 被新调用取代，退出
      if (_syncGeneration != generation) return;
      // 检查是否存在数据断层风险
  ...
  Future<void> _subscribeSocket({required int generation}) async {
    if (_isDisposed || !_isStarted) return;
    ...
    try {
      ...
      // 校验：await 期间 socket 可能已断开，timestamp 已过时
      if (!(_sharedPriceSubscription?.isConnected.value ?? false)) {
        logger.w('CandleDataSynchronizer: connectReady 返回但连接已断开，时间戳作废');
        _socketConnectedTime = null;
      }
    } on TimeoutException {
      logger.w('CandleDataSynchronizer: socket 连接超时');
      _socketConnectedTime = null;
    }

    // 被新调用取代，退出（不再设置 listener）
    if (_syncGeneration != generation) return;
    ...
    try {
      ...
      _initialLoadState = UiState.failure(
        ...
        retry: () {
          ...
          _loadInitialData(generation: gen, isRefresh: isRefresh);
        },
      );
      _onInitialLoadStateChanged?.call(_initialLoadState);
    }
  ...
  void _onPriceData(ManicPriceData priceData) {
    ...
    // 检查是否是更新现有蜡烛还是添加新蜡烛
    if (_candles.isEmpty || newCandle.epoch > _candles.last.epoch) {
      // 添加新蜡烛
      _candles.add(newCandle);
    } else if (_candles.last.epoch == newCandle.epoch) {
      // 更新最后一根蜡烛
      _candles[_candles.length - 1] = newCandle;
    } else {
      // epoch < last.epoch 的乱序数据直接丢弃
      return;
    }
    _onCandlesChanged?.call(_candles.toList());
  }
  ...
  /// 刷新数据
  ///
  /// [forceReconnect] 是否强制重建底层 WebSocket。
  /// - false（默认，用户下拉刷新）：复用现有共享连接，只重新拉取 HTTP 数据
  /// - true（回前台强制刷新等场景）：先请求底层 socket 重连再做全量同步，
  ///   用于规避 TCP 静默断开导致的僵尸连接
  Future<void> refreshData({bool forceReconnect = false}) async {
    if (_isDisposed || !_isStarted) return;

    if (forceReconnect) {
      logger.d('CandleDataSynchronizer: 强制刷新，请求底层 socket 重连');
      // 必须 await：确保旧 socket 已 disconnect、connectCompleter 已重置，
      // 否则后续 _subscribeSocket 的 connectReady 可能立刻返回旧时间戳
      await _sharedPriceSubscription?.requestReconnect();
      if (_isDisposed || !_isStarted) return;
    } else {
    ...
    await _loadDataWithSocketSync(sequential: true, isRefresh: true);
  }
}
