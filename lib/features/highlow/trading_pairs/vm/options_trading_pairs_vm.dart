...
class OptionsTradingPairsVM extends ViewModel {
  ...
  Future<void> _fetchTradingPairs(bool isInitial) async {
    ...
    try {
      if (isInitial) {
        logger.d('[TradingPair] 开始从本地获取交易对信息, isInitial: $isInitial');
      ...
      logger.d('[TradingPair] 开始从远程获取交易对信息, isInitial: $isInitial');
      var remoteTradingPairDetails =
          await _repository.fetchTradingPairDetails();
      logger
          .d('[TradingPair] 远程获取到的交易对信息数量: ${remoteTradingPairDetails.length}');
  ...
  Future<void> refreshTradingPairs() async {
    if (!_tradingPairsState.value.isLoading) {
      await _fetchTradingPairs(_tradingPairsState.value.isInitial);
    }
  }
