...
final activityVMProvider = ViewModelProvider<ActivityVM>((ref) {
  return ActivityVM(injector(), ref.bind(openedPositionsVMProvider));
});
...
class ActivityVM extends ViewModel {
  ...
  ActivityVM(this._dataSource, this._openedPositionsVM);
  ...
  Future<List<ActivityItem>> _loadPositionHistory() async {
    ...
    try {
      ...
      // 将每条交易记录拆分为开仓和结算两条
      // 仍在未结算列表中的仓位不展示结算项（避免后端 status 尚未回写时误判）
      final openedIds = _openedPositionsVM.openedPositions.value
          .map((p) => p.id)
          .toSet();
      final List<ActivityItem> items = [];
      for (final item in response.nodes) {
        items.add(OpenPositionActivityItem(item));
        if (!openedIds.contains(item.positionId)) {
