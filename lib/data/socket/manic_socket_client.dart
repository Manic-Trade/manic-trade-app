...
class ManicSocketClient {
  ...
  /// 最近一次收到任何消息（包括 price/原始消息）的时间戳（毫秒）
  /// 用于上层做僵尸连接探测
  int? _lastMessageAtMs;
  int? get lastMessageAtMs => _lastMessageAtMs;
  ...
  void _onMessage(dynamic data) {
    _lastMessageAtMs = DateTime.now().millisecondsSinceEpoch;
    if (data is String) {
  ...
  /// 强制重连：先断开再用当前参数重新连接
  /// 用于僵尸连接（TCP 静默断开）场景，绕过 connect() 的已连接短路
  Future<void> forceReconnect() async {
    if (_disposed) return;
    final asset = _currentAsset;
    final timeframe = _currentTimeframe;
    if (asset == null) return;
    logger.d('ManicSocketClient: 强制重连 asset=$asset timeframe=$timeframe');
    await disconnect();
    if (_disposed) return;
    await connect(asset: asset, timeframe: timeframe);
  }
