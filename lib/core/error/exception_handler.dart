...
class ErrorHandler {
  ...
  static String getMessage(BuildContext context, dynamic error,
    ...
    if (error is String) {
      ...
      return getMessage(context, error.error, fallback: fallback);
    } else if (error is CancelError) {
      return context.strings.cancelled;
    } else if (error is MissingPluginException) {
      return error.message ?? fallback ?? context.strings.error_default;
      ...
      return fallback ?? context.strings.error_default;
    }
  }
}
