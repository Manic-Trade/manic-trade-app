...
class _BounceTapperState extends State<BounceTapper>
  ...
  static const double _horizontalDragThreshold = 8.0; // 水平滑动阈值，可以根据需要调整
  ...
  Widget build(BuildContext context) {
    return Listener(
        ...
        try {
          ...
          if (_initialPosition != null) {
            ...
            if (!_isHorizontalDrag &&
                offset.dx.abs() > _horizontalDragThreshold &&
          ...
          if (_isHorizontalDrag ||
              !_isPointerWithinBounds(event.localPosition)) {
      ...
      onPointerUp: (event) async {
        try {
          if (!widget.enable || _targetPoint != event.pointer) {
            return;
          }
          // 立即取消长按定时器
          _longPressTimer?.cancel();
          ...
          Future.microtask(() async {
            ...
            } else if (widget.onTap != null && !_isLongPressed) {
  ...
  void dispose() {
    try {
      ...
      log('catch Exception on Dispose state', error: e);
    }

    super.dispose();
  }

  bool _isPointerWithinBounds(Offset position) {
    final size = _touchAreaKey.currentContext?.size ?? Size.zero;
    return isWithinBounds(position: position, touchAreaSize: size);
  }
}
